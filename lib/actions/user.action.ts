...
export async function createUser(userData: CreateUserParams) {
  try {
    ...
    // Ensure username is never empty, null, or undefined
    if (!userData.username || userData.username.trim() === "" || userData.username === null) {
      const emailPrefix = userData.email?.split("@")[0];
      userData.username = emailPrefix || `user_${Date.now()}`;
    }
